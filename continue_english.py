...
all_cards = [card["id"] for sec_id, data in section_cards.items() for card in data["cards"]]
...
remaining = [c for c in all_cards if c not in answers]
print(f"Done: {len(answers)}, Remaining: {len(remaining)}", flush=True)
...
while togo:
    ...
    with ThreadPoolExecutor(max_workers=50) as ex:
        fs = {ex.submit(fetch, c): c for c in batch}
